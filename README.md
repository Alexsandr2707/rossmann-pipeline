...
После data quality этапа `update` обучает регрессионные модели из `model.candidate_models`.
...
Метрики регрессии: `rmse`, `mae`, `r2`, `smape`. Основная метрика выбирается через `model.primary_metric`, сейчас это `rmse`.
